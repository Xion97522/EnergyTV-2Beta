...
import { X, Play, ChevronLeft, ExternalLink, Tv, Layers, AlertTriangle, RefreshCw, Loader2 } from "lucide-react";
...
export default function PlayerModal({ media, onClose, initialSeason = 1, initialEpisode = 1 }: PlayerModalProps) {
  ...
  const [step, setStep]             = useState<"season" | "episode" | "player">(isTV ? "season" : "player");
  const [loading, setLoading]       = useState(true);
  const [timedOut, setTimedOut]     = useState(false);

  const iframeKey   = useRef(0); // bump to force remount
  ...
  /* Reset loading state whenever source / episode changes */
    ...
    >
      <div
        ...
        style={{
          ...
          border: "1px solid rgba(255,255,255,0.09)",
          boxShadow: "inset 0 1.5px 0 rgba(255,255,255,0.1), 0 30px 90px rgba(0,0,0,0.8), 0 0 0 1px rgba(57,255,20,0.06)",
        }}
      >
        {/* Specular top edge */}
        <div className="absolute top-0 left-0 right-0 h-px pointer-events-none" style={{ background: "linear-gradient(90deg,transparent,rgba(255,255,255,0.12) 35%,rgba(57,255,20,0.12) 65%,transparent)" }} />

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.055)" }}>
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="p-1.5 rounded-xl transition-all hover:scale-105" style={GLASS_BTN}>
          ...
          <div className="flex items-center gap-2">
            {isTV && step === "player" && (
              <button onClick={() => setStep("episode")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-white/60 transition-all" style={GLASS_BTN}>
                <Tv className="w-3.5 h-3.5" /> Episodes
              </button>
            )}
            <button onClick={openExternal} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-white/60 transition-all hover:text-white/90" style={GLASS_BTN}>
        ...
        {step === "season" && (
          <div className="p-6">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2.5">
              {allSeasons.map((s) => (
                <button key={s} onClick={() => { setSeason(s); setStep("episode"); }} className="py-3.5 rounded-2xl font-bold text-sm transition-all hover:scale-105 active:scale-95"
                  style={s === season ? { background: "linear-gradient(135deg,hsl(112,100%,54%),hsl(112,100%,38%))", color: "#000", boxShadow: "0 0 18px rgba(57,255,20,0.4)", border: "none" }
          ...
          <div className="p-5">
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto hide-scrollbar">
              {(localEps.length > 0
                ...
                <button key={ep.num} onClick={() => { setEpisode(ep.num); setStep("player"); }}
                  className="w-full flex items-center gap-3 p-3.5 rounded-2xl text-left transition-all group/ep"
                  style={{ ...GLASS_BTN, background: episode === ep.num ? "rgba(57,255,20,0.08)" : "rgba(255,255,255,0.03)", borderColor: episode === ep.num ? "rgba(57,255,20,0.2)" : "rgba(255,255,255,0.06)" }}>
                  <div className="shrink-0 w-8 h-8 rounded-xl flex items-center justify-center text-xs font-black"
                    style={{ background: episode === ep.num ? "rgba(57,255,20,0.2)" : "rgba(255,255,255,0.06)", color: episode === ep.num ? "hsl(112,100%,54%)" : "rgba(255,255,255,0.45)" }}>
                    {ep.num}
                  </div>
        ...
        {step === "player" && (
          <>
            <div className="relative" style={{ aspectRatio: "16/9", background: "#000" }}>
              ...
              {loading && !timedOut && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 z-10"
                  ...
                  <p className="text-xs text-muted-foreground/60">Loading <span className="font-semibold text-foreground/60">{source.label}</span>…</p>
                </div>
              )}
            ...
            {/* Source switcher bar */}
            <div className="flex items-center gap-2 px-4 py-3 flex-wrap" style={{ borderTop: "1px solid rgba(255,255,255,0.055)" }}>
              ...
              {isTV && (
                <button onClick={() => setStep("episode")} className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-semibold text-white/45 ml-1" style={GLASS_BTN}>
